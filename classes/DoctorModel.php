...
class DoctorModel{
    ...
    function deletePatient($id){
        ...
        if($query->execute($input)){
            header('Location:../doctor/dashboardDoctor5.php');
        }else{echo "Ntorr!";}
    }
    ...
    function addDiagnosis($admission, $details, $doctor, $patient){
        ...
        if($query->execute()){
            echo "<script>window.location='../doctor/dashboardDoctor2.php?admissionid=".$admission."&patientid=".$patient."'</script>";
        }
    }
    ...
    function addPres($pi, $p, $admission, $pd, $pid){
        ...
        if($query->execute()){
             echo "<script>window.location='../doctor/dashboardDoctor2.php?admissionid=".$admission."&patientid=".$pid."'</script>";
        }
    }
    ...
    function removePres($pid, $admission, $pi){
        ...
        if($query->execute()){
             echo "<script>window.location='../doctor/dashboardDoctor2.php?admissionid=".$admission."&patientid=".$pi."'</script>";
        }
    }
    ...
    function Bill($input){
        ...
        $query = $conn->prepare($sql);
        if($query->execute($input)){
            echo "<script>window.location='../doctor/dashboardDoctor3.php'</script>";
        }
    }
    ...
    function submitQ($qid, $reply, $sid){
        ...
        if($query->execute()){
            ...
            echo "<script>window.location='../doctor/dashboardDoctor4.php'</script>";
        }
    }
